...
def create_log(
    ...
    role_names = ", ".join(
        str(role_name) for role_name in decoded_token.get("role_names", "")
    )
    ...
    log_data = {
        ...
        "payloadCreatedDate": datetime.datetime.strptime(
            user_request.created_at, "%Y-%m-%dT%H:%M:%S.%f%z"
        ...
        "lastLogin": datetime.datetime.strptime(
            user_request.last_login, "%Y-%m-%dT%H:%M:%S.%f%z"
        ).strftime("%Y-%m-%d %H:%M:%S.%f"),
    ...
    headers = {"Content-Type": "application/json", "Authorization": token_key}
    try:
        response = requests.post(url=logger_url, headers=headers, json=log_data)
        response.raise_for_status()
    except Exception as e:
        print(
            "sterling_shared.create_log",
...
def push_audit_log(data: AuditLogData):
    ...
    try:
        ...
        print("sterling_shared.push_audit_log", response.status_code, response.text)
        # response.raise_for_status()
